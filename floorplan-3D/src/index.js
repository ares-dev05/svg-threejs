...
function selectFloorTexture(data) {
  ...
  if (floor_texture_pack.colormap) {
    settingsSelectedRoom3D.setValue(
      "Floor Texture:",
      floor_texture_pack.colormap
    );
  } else {
...
function selectWallTexture(data) {
  ...
  if (settingsSelectedWall3D._hidden && !settingsSelectedRoom3D._hidden) {
    ...
    if (colormap) {
      settingsSelectedWall3D.setValue(
        "Wall Texture:",
        wall_texture_pack.colormap
      );
    } else {
...
function exportDesignAsPackage() {
  function getWallTextureImages(texobject, pre_image_paths) {
    ...
    if (
      texobject.roughnessmap &&
      !pre_image_paths.includes(texobject.roughnessmap)
    ) {
      image_paths.push(texobject.roughnessmap);
    }
    if (
      texobject.ambientmap &&
      !pre_image_paths.includes(texobject.ambientmap)
    ) {
  ...
  function writeZip() {
    if (
      !fetched_image_files.length === images.length &&
      !fetched_model_files.length === models.length
    ) {
      return;
    }
  }
...
blueprint3d.floorplanner.addFloorplanListener(
  EVENT_NOTHING_2D_SELECTED,
  function () {
    settingsSelectedCorner.hide();
    settingsSelectedWall.hide();
...
blueprint3d.floorplanner.addFloorplanListener(
  ...
  function (evt) {
    ...
    settingsSelectedCorner.setValue(
      "cornerElevation",
      Dimensioning.cmToMeasureRaw(evt.item.elevation)
    );
  }
);
blueprint3d.floorplanner.addFloorplanListener(
  ...
  function (evt) {
    ...
    settingsSelectedRoom.hide();
    settingsViewer2d.showControl("Delete");
    settingsSelectedWall.setValue(
      "wallThickness",
      Dimensioning.cmToMeasureRaw(evt.item.thickness)
    );
  }
);
blueprint3d.floorplanner.addFloorplanListener(
...
blueprint3d.roomplanner.addRoomplanListener(
  EVENT_ITEM_SELECTED,
  function (evt) {
    settingsSelectedWall3D.hide();
    settingsSelectedRoom3D.hide();
    ...
    if (itemModel.isParametric) {
      parametricContextInterface = new ParametricsInterface(
        itemModel.parametricClass,
...
if (!opts.widget) {
  ...
  settingsViewer2d.addButton(
    "Delete",
    floorplanningHelper.deleteCurrentItem.bind(floorplanningHelper)
  );

  settingsViewer2d.bindBoolean(
    "snapToGrid",
    configurationHelper.snapToGrid,
    configurationHelper
  );
  settingsViewer2d.bindBoolean(
    "directionalDrag",
    configurationHelper.directionalDrag,
    configurationHelper
  );
  settingsViewer2d.bindBoolean(
    "dragOnlyX",
    configurationHelper.dragOnlyX,
    configurationHelper
  );
  settingsViewer2d.bindBoolean(
    "dragOnlyY",
    configurationHelper.dragOnlyY,
    configurationHelper
  );
  settingsViewer2d.bindRange(
    "snapTolerance",
    1,
    200,
    configurationHelper.snapTolerance,
    1,
    configurationHelper
  );
  settingsViewer2d.bindRange(
    ...
    10,
    200,
    configurationHelper.gridSpacing,
    1,
    configurationHelper
  );
  settingsViewer2d.bindNumber(
    "boundsX",
    1,
    200,
    configurationHelper.boundsX,
    1,
    configurationHelper
  );
  settingsViewer2d.bindNumber(
    ...
    1,
    200,
    configurationHelper.boundsY,
    1,
    configurationHelper
  );

  settingsSelectedCorner.bindRange(
    "cornerElevation",
    1,
    500,
  ...
  settingsSelectedWall.bindRange(
    "wallThickness",
    0.01,
    1,
    floorplanningHelper.wallThickness,
    0.01,
    floorplanningHelper
  );
  settingsSelectedRoom.bindText(
    "roomName",
    floorplanningHelper.roomName,
    floorplanningHelper
  );
  ...
  console.log(
    "TEXTURE IMAGE :: ",
    floor_textures[floor_texture_keys[0]].colormap || TEXTURE_NO_PREVIEW
  );
  settingsSelectedRoom3D.addDropDown(
    "Floor Textures",
    floor_texture_keys,
    selectFloorTexture
  );
  settingsSelectedRoom3D.addImage(
    "Floor Texture:",
    floor_textures[floor_texture_keys[0]].colormap || TEXTURE_NO_PREVIEW,
    null
  );
  settingsSelectedRoom3D.addColor(
    "Floor Texture Color:",
    floor_textures[floor_texture_keys[0]].color || "#FFFFFF",
    selectFloorTextureColor
  );
  ...
  settingsSelectedRoom3D.addImage(
    "All Wall Texture:",
    wall_textures[wall_texture_keys[0]].colormap || TEXTURE_NO_PREVIEW,
    selectWallTexture
  );
  settingsSelectedRoom3D.addColor(
    "All Wall Texture Color:",
    wall_textures[wall_texture_keys[0]].color || "#FFFFFF",
    selectWallTextureColor
  );
  settingsSelectedRoom3D.addButton("Apply", selectWallTexture);

  settingsSelectedWall3D.addDropDown(
    "Wall Textures",
    wall_texture_keys,
    selectWallTexture
  );
  settingsSelectedWall3D.addImage(
    "Wall Texture:",
    wall_textures[wall_texture_keys[0]].colormap || TEXTURE_NO_PREVIEW,
    null
  );
  settingsSelectedWall3D.addColor(
    "Wall Texture Color:",
    wall_textures[wall_texture_keys[0]].color || "#FFFFFF",
    selectWallTextureColor
  );
  ...
  settingsViewer3d.addHTML(
    "Tips:",
    "<p>Click and drag to rotate the room in 360\xB0</p><p>Add room items <ul><li>Add parametric doors</li><li>Other items (Coming soon)</li></ul></p><p>Drag and Place items(pink boxes and parametric doors) in the room</p><p>There are 8 different types of items <ul><li>1: FloorItem</li> <li>2: WallItem</li> <li>3: InWallItem</li> <li>7: InWallFloorItem</li> <li>8: OnFloorItem</li> <li>9: WallFloorItem</li><li>0: Item</li> <li>4: RoofItem</li></ul></p>"
  );
  ...
  settingsViewer2d.setPosition(
    app_parent.clientWidth - panelWidths,
    startY + uxInterfaceHeight
  );
  settingsViewer3d.setPosition(
    app_parent.clientWidth - panelWidths,
    startY + uxInterfaceHeight
  );
